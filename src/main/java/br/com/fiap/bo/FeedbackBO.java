...
public class FeedbackBO {
    ...
    public FeedbackTO save(FeedbackTO feedback) {
        ...
        if (feedback.getComentario() == null || feedback.getComentario().trim().isEmpty()) {
            System.out.println("Falha ao salvar feedback: O comentário não pode ser vazio.");
            return null;
        }

        if (feedback.getAvaliacao() < 1 || feedback.getAvaliacao() > 5) {
            System.out.println("Falha ao salvar feedback: A avaliação deve ser um número entre 1 e 5.");
            return null;
        }
        if (feedback.getIdUsuario() == 0) {
            System.out.println("Falha ao salvar: O feedback não está vinculado a um usuário.");
            return null;
        }
    ...
    public boolean delete(Integer idFeedback, Integer idUsuarioLogado) {
        ...
        if (feedback == null) {
            System.out.println("Falha ao excluir: Feedback com ID " + idFeedback + " não encontrado.");
            return false;
        }


        if (feedback.getIdUsuario() != idUsuarioLogado) {
            System.out.println("Falha ao excluir: Usuário " + idUsuarioLogado + " não tem permissão para excluir o feedback " + idFeedback + ".");
            return false;
        }
